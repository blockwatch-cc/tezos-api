...
ARTIFACT := tzstats_docs_api
ifdef TZSTATS_DOCS_VERSION
	BUILD_VERSION := $(TZSTATS_DOCS_VERSION)
endif
ifndef BUILD_ID
...
image: build
	...
	@docker build --pull --rm --no-cache -f $(DOCKERFILE) --build-arg BUILD_DATE=$(BUILD_DATE) --build-arg BUILD_VERSION=$(BUILD_VERSION) --build-arg BUILD_ID=$(BUILD_ID) -t $(TARGET_IMAGE) -t $(TARGET_LATEST) .
	@echo
